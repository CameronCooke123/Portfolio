    ...
    while True:
        ...
        for proc in psutil.process_iter():
            try:
                ...
                usr = usr.replace(socket.gethostname() + "\\", "",
                                  1)  # removes hostname (device name) from before username.
                ...
                if pid != 0:  # pid 0 is a dummy process initiated by windows that causes errors
                    ...
                    if name != 'svchost.exe':
                        pslst.append(
                            f'{timestamp},{machine_id},{name},{mempct},{cpupct},{memabs} MB,{numthd},{usr},{path},{pid}')
            except:
                if debug:
